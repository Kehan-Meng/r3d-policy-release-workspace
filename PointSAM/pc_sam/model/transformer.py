...
class TwoWayTransformer(nn.Module):
    ...
    def forward(
        ...
        Args:
          ...
          key_padding_mask (torch.Tensor): [B, N_pc_tokens] bool mask.
            True = valid token, False = padding. Applied in cross-attention from
            point_embedding to pc_embedding.
...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # Re-zero padding text positions after they were updated as queries.
        if key_padding_mask is not None:
            keys = keys * key_padding_mask.unsqueeze(-1).to(keys.dtype)
...
class Attention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor,
        ...
        # Mask padding key positions before softmax.
        # key_padding_mask: [B, L_k], True = valid, False = padding.
        if key_padding_mask is not None:
