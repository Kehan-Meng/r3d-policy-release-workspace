# Generate Adroit expert demonstrations with a pretrained VRL3 expert.
import os
import shutil
import sys
...
VRL3_ROOT = Path(
    os.environ.get(
        "R3D_VRL3_ROOT",
        RELEASE_ROOT / "third_party" / "r3d_benchmarks" / "third_party" / "VRL3",
    )
).expanduser().resolve()
VRL3_SRC = VRL3_ROOT / "src"


def load_runtime_dependencies():
    if not VRL3_SRC.is_dir():
        raise RuntimeError(
            f"VRL3 source not found at {VRL3_SRC}. Run "
            "environment/install_benchmarks.sh adroit or set R3D_VRL3_ROOT."
        )
    if str(VRL3_SRC) not in sys.path:
        sys.path.insert(0, str(VRL3_SRC))

    # Imported lazily so --help works before benchmark installation.
    global AdroitEnv, MujocoPointcloudWrapperAdroit, cprint, utils
    import mj_envs  # noqa: F401 - registers the Adroit environments
    from adroit import AdroitEnv
    import utils
    from termcolor import cprint
    from r3d.gym_util.mjpc_wrapper import MujocoPointcloudWrapperAdroit
