...
def point_cloud_sampling(point_cloud:np.ndarray, num_points:int, method:str='fps'):
    ...
    if method == 'uniform':
        ...
        point_cloud_tensor = torch.from_numpy(point_cloud).unsqueeze(0).to(torch_device)
        num_points_tensor = torch.tensor([num_points], dtype=torch.int64, device=torch_device)
        try:
