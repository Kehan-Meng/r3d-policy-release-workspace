...
class ManiskillRunner(BaseRunner):
    ...
    def run(self, policy: BasePolicy, epoch=None, task_config=None):
        ...
        for episode_idx in range(self.eval_episodes):
            # start rollout
            base_seed = 100000 if self.deterministic_eval_seed is None else int(self.deterministic_eval_seed)
            obs = env.reset(seed=base_seed + self.episode_start + episode_idx)
            policy.reset()
        ...
        if self.task_name == 'PegInsertionSide-v0' and final_eval_num:
            base_env = env.env
            log_data.update({
                'grasp_success_rate': base_env.is_grasped_success / final_eval_num,
                'align_success_rate': base_env.pre_inserted_success / final_eval_num,
                'insert_success_rate': final_success_num / final_eval_num,
            })
