...
from r3d.gym_util.mujoco_point_cloud import PointCloudGenerator
from r3d.gym_util.mjpc_wrapper import point_cloud_sampling
...
class MetaWorldEnvFast(MetaWorldEnv):
    """Fast eval mode: skip RGB image rendering and video recording.

    Only computes point_cloud and agent_pos — the two observations the
    policy actually consumes.  Removes ~2 redundant EGL renders per step,
    roughly halving GPU contention during multi-eval.
    """
    ...
    def get_video(self):
        import numpy as np
        return np.zeros((1, 3, self.image_size, self.image_size), dtype=np.uint8)
