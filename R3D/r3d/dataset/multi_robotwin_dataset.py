...
class MultiRobotwinDataset(BaseDataset):
    def __init__(self,
            ...
            agent_pos_perturb_std=0.0,  # S6: recovery perturbation
        ...
        ) and instruction_aug.get("enabled"):
        ...
        self.agent_pos_perturb_std = agent_pos_perturb_std  # S6: recovery perturbation
        ...
        # Different domains/settings of the same semantic task share one
        # condition id. A per-entry id leaks the source dataset and makes the
        # condition used at evaluation ambiguous.
        self.task_names = list(dict.fromkeys(
            str(task.get("task_name", idx)) for idx, task in enumerate(self.tasks)
        ))
        self.task_name_to_idx = {
            task_name: idx for idx, task_name in enumerate(self.task_names)
        }
        self.entry_to_task_idx = [
            self.task_name_to_idx[str(task.get("task_name", idx))]
            for idx, task in enumerate(self.tasks)
        ]
        self.num_tasks = len(self.task_names)
        self.task_to_idx = {
            self._task_key(task, idx): self.entry_to_task_idx[idx]
            for idx, task in enumerate(self.tasks)
        }
    ...
    def get_validation_dataset(self):
        ...
        val_set.training = False  # disable instruction augmentation for val
        return val_set
    ...
    def get_normalizer(self, mode="limits", **kwargs):
        if self.frame_transform_enabled:
            ...
            for item in self.task_datasets:
                ...
                sources.append(FrameNormalizerSource(
                    arrays={
                        "action": action,
                    ...
                    episode_mask=item["train_mask"],
                    source_id=str(item["task"].get("zarr_path")),
                ))
            return self._fit_frame_aware_normalizer(
                sources,
                mode=mode,
                identity_fields={"task_onehot": self.num_tasks},
                **kwargs,
            )
        actions = []
        agent_pos = []
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        entry_idx, local_idx = self.indices[idx]
        item = self.task_datasets[entry_idx]
        sample = item["sampler"].sample_sequence(local_idx)
        task_idx = self.entry_to_task_idx[entry_idx]
        ...
        if self.use_data_augmentation:
            ...
            if "agent_pos" in data["obs"]:
                ...
                # S6: recovery-style perturbation
                if self.agent_pos_perturb_std > 0:
                    perturb = np.random.randn(
                        *data["obs"]["agent_pos"].shape
                    ).astype(np.float32) * self.agent_pos_perturb_std
                    perturb[..., 6] = 0.0
                    perturb[..., 13] = 0.0
                    data["obs"]["agent_pos"] = data["obs"]["agent_pos"] + perturb
        ...
        command = item["task"].get("text_command") or item["task"].get("task_name")
