"""Zarr dataset adapter for the Franka grasp peg-assembly benchmark.

The collector contract follows the common R3D single-task layout::

    data/point_cloud  float32 [T, N, 6]
    data/state        float32 [T, 34]
...
class FrankaGraspPegAssemblyDataset(BaseDataset):
    """Load successful grasp-peg-assembly demonstrations for behavior cloning.

    Compared with ``PegAssemblyDataset`` this class expects the **new**
    franka_grasp_peg_assembly data contract: 34-D state and 7-D action.
    """
    ...
    def __init__(
        ...
        if self.state_dim != 34:
            raise ValueError(
                "FrankaGraspPegAssemblyDataset state_dim must be 34; legacy "
                f"30-D data belongs to PegAssemblyDataset, got {self.state_dim}"
            )
        self.point_cloud_num_points = int(point_cloud_num_points)
        self.point_cloud_shape = (self.point_cloud_num_points, 6)
    ...
    def _validate_replay_contract(self):
        ...
        if episode_ends_raw.dtype != np.dtype(np.int64):
            raise TypeError(
                "FrankaGraspPegAssemblyDataset meta/episode_ends must be int64, "
                f"got {episode_ends_raw.dtype}"
            )
        episode_ends = episode_ends_raw.astype(np.int64, copy=False)
        if episode_ends.ndim != 1 or episode_ends.size == 0:
            raise ValueError(
                "FrankaGraspPegAssemblyDataset requires at least one complete "
                f"episode in meta/episode_ends, got shape {episode_ends.shape}"
            )
        if episode_ends[0] <= 0 or np.any(np.diff(episode_ends) <= 0):
            raise ValueError(
                "FrankaGraspPegAssemblyDataset meta/episode_ends must be positive "
                f"and strictly increasing, got {episode_ends.tolist()}"
            )
        ...
        for key, trailing_shape in expected.items():
            ...
            if actual != trailing_shape:
                raise ValueError(
                    f"FrankaGraspPegAssemblyDataset expected data/{key} trailing "
                    f"shape {trailing_shape}, got {actual} in {self.zarr_path}"
                )

            values = np.asarray(self.replay_buffer[key])
            if values.dtype != np.dtype(np.float32):
                raise TypeError(
                    f"FrankaGraspPegAssemblyDataset data/{key} must be float32, "
                    f"got dtype {values.dtype}"
                )
            if not np.isfinite(values).all():
                raise ValueError(
                    f"FrankaGraspPegAssemblyDataset data/{key} contains NaN or "
                    f"infinite values"
                )
        ...
        if (
            ...
            raise ValueError(
                "FrankaGraspPegAssemblyDataset expects normalized actions in "
                f"[-1, 1]; observed [{float(actions.min()):.6g}, "
    ...
    def _validate_episode_shapes(self, expected_shape):
        ...
        if "meta" not in root or "shape" not in root["meta"]:
            raise ValueError(
                "FrankaGraspPegAssemblyDataset expected meta/shape so the "
                f"square-only training protocol can be enforced: {self.zarr_path}"
            )
        ...
        if actual != {expected_shape}:
            raise ValueError(
                "FrankaGraspPegAssemblyDataset shape protocol violation: "
                f"expected only {expected_shape!r}, found {sorted(actual)} in "
                f"{self.zarr_path}"
            )

    def _validate_collection_distribution(self, expected_distribution):
        """Keep wider pose-OOD demonstrations out of the train split."""

        manifest_path = Path(self.zarr_path) / "generation_metadata.json"
        if not manifest_path.is_file():
            raise ValueError(
                "FrankaGraspPegAssemblyDataset expected "
                f"generation_metadata.json: {self.zarr_path}"
            )
        ...
        if actual != expected_distribution:
            raise ValueError(
                "FrankaGraspPegAssemblyDataset reset-distribution protocol "
                f"violation: expected {expected_distribution!r}, found "
                f"{actual!r} in {self.zarr_path}"
            )
        if expected_distribution == "train" and collector.get(
            "fixed_reset_options"
        ) is not None:
            raise ValueError(
                "FrankaGraspPegAssemblyDataset train data must use independently "
                "sampled resets, not fixed_reset_options"
            )
        if expected_distribution not in RESET_DISTRIBUTION_RANGES:
            raise ValueError(
                f"Unknown reset distribution {expected_distribution!r}"
            )
        ...
        if actual_env_distribution != expected_env_distribution:
            raise ValueError(
                "FrankaGraspPegAssemblyDataset collector.env_kwargs.distribution "
                f"mismatch: expected {expected_env_distribution!r}, found "
        ...
        if np.any(np.abs(reset_options[:, :5]) > upper + tolerance) or np.any(
            ...
            raise ValueError(
                "FrankaGraspPegAssemblyDataset reset options exceed the declared "
    ...
    def get_normalizer(self, mode="limits", **kwargs):
        ...
        if last_n_dims != 1:
            raise ValueError(
                "FrankaGraspPegAssemblyDataset normalizer expects last_n_dims=1, "
                f"got {last_n_dims}"
            )
        arrays = {
            "action": self.replay_buffer["action"],
