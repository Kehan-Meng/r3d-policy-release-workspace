...
class RobotwinDataset(BaseDataset):
    def __init__(self,
            ...
            agent_pos_perturb_std=0.0,  # S6: recovery-style perturbation (rad)
            task_name=None,
            use_endpose=False,
        ...
        self.instruction_bank = None  # set externally or via config
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.use_data_augmentation:
            ...
            if 'agent_pos' in data['obs']:
                # Add small Gaussian noise (existing)
                data['obs']['agent_pos'] = add_noise(
                    data['obs']['agent_pos'],
                    noise_std=self.agent_pos_noise_std,
                    clip_range=2*self.agent_pos_noise_std
                )
                # S6: recovery-style perturbation — simulate model-induced state deviation
                if self.agent_pos_perturb_std > 0:
                    ...
                    # Only perturb arm joints [0:6] and [7:13], not gripper [6] and [13]
                    perturb[..., 6] = 0.0
                    perturb[..., 13] = 0.0
